...
'''
units = [80,40,20] # hidden layer units number
...
model = Linear_model.double() # both model and data use double precision
model.to(device)
criterion = nn.CrossEntropyLoss() # loss function: CrossEntropyLoss, default to get averaged loss
lrate = 1e-2
momentum = 0.9
optimizer = torch.optim.SGD(model.parameters(), lr=lrate, momentum = momentum) #default momentum is 0
...
#test_loader = DataLoader(D_test, batch_size = test_size, shuffle = False)
...
if SaveOrNot == True:
    ...
    fileDir = os.path.join(codeDir,
                           'result\\pytorch linear classifier CrossEntropyLoss SGD\\batch{:.0f}_lr{:.0e}_momentum{:.1f}_epoch{:.0f}'
                           .format(batchsize,lrate,momentum,epochs))
    if not os.path.exists(fileDir):
        os.makedirs(fileDir)
    head = 'pytorch linear classifier ReLU CrossEntropyLoss SGD:\n batch={:.0f},lr={:.0e},momentum={:.1f},epoch={:.0f}'\
            .format(batchsize,lrate,momentum,epochs)
...
if PlotOrNot == True :
    ...
    figure_title = 'pytorch linear classifier ReLU CrossEntropyLoss SGD:\n batch={:.0f},lr={:.0e},momentum={:.1f}'\
                    .format(batchsize,lrate,momentum)
    fig.suptitle(figure_title)
    axs[0,0].set(ylabel='Training Loss',ylim=[0.2,0.8])
    axs[0,1].set(ylabel='Validation Loss',ylim=[0.2,0.8])
    axs[1,0].set(xlabel='epoch',ylabel='Training Accuracy',ylim=[0.4,0.95])
    axs[1,1].set(xlabel='epoch',ylabel='Validation Accuracy',ylim=[0.4,0.95])
    for k in range(K):
