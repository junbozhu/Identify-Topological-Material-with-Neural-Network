...
epochs = 128 # one epoch is a loop of whole train_dataset
...
units = [40] # hidden layer units number
...
model = nn_onelayer_Relu.double() # both model and data use double precision
model.to(device)
...
if SaveOrNot == True:
    ...
    fileDir = os.path.join(codeDir,
                           'result\\pytorch neural network ReLU CrossEntropyLoss SGD\\unit{:s}_batch{:.0f}_lr{:.0e}_momentum{:.1f}_epoch{:.0f}'
                           .format(str(units),batchsize,lrate,momentum,epochs))
    if not os.path.exists(fileDir):
        os.makedirs(fileDir)
    head = 'pytorch neural network ReLU CrossEntropyLoss SGD:\n unit={:s},batch={:.0f},lr={:.0e},momentum={:.1f},epoch={:.0f}'\
            .format(str(units),batchsize,lrate,momentum,epochs)
...
if PlotOrNot == True :
    ...
    figure_title = 'pytorch neural network ReLU CrossEntropyLoss SGD:\n unit={:s},batch={:.0f},lr={:.0e},momentum={:.1f}'\
                    .format(str(units),batchsize,lrate,momentum)
